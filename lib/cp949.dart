...
List<int> toUnicodes(final List<int> codeUnits) {
  final List<int> unicodes = List.filled(codeUnits.length, 0);
  var j = 0;
...
String decode(final List<int> codeUnits) {
  ...
  final unicodeData = toUnicodes(codeUnits);

  if (unicodeData.contains(null)) {
...
List<int> encode(final String str) {
  final List<int> cp949codeUnits = List.filled(str.codeUnits.length * 2, 0);
  var i = 0;
  for (final unicode in str.codeUnits) {
    ...
    if (cp949Code != null) {
      firstByte = cp949Code >> 8;
      secondByte = cp949Code - (firstByte << 8);
    }
...
String encodeToString(final String str) {
  final encodeData = encode(str);

  if (encodeData.contains(null)) {
    throw NullThrownError();
  }
